...
no_lookup_done (FlowIPService *ip_service)
{
  ...
  {
    /* The resulting state is bad (missing data that would have been generated by a
     * successful lookup), so generate an error. */
    priv->resolve_error = g_error_new_literal (FLOW_LOOKUP_DOMAIN_QUARK, FLOW_LOOKUP_NO_RECORDS,
...
static void
lookup_done (GList *addr_list, GList *name_list, GError *error, FlowIPService *ip_service)
{
  FlowIPServicePrivate *priv    = ip_service->priv;
  ...
  else
  {
    ...
    /* FIXME: If the new values are complete, maybe they should replace the
     * old ones? */

    /* Don't propagate an error if result is okay */
    success = TRUE;
  }
  ...
  if (!success)
  {
    /* priv->resolve_error gets cleared when lookup is scheduled */
    priv->resolve_error = error;
  }
  else if (error)
  {
    /* The resulting state is okay (we have both a name and addresses), so don't
     * propagate an error. */
    g_error_free (error);
  }
...
gboolean
flow_ip_service_sync_resolve (FlowIPService *ip_service, GError **error)
{
  FlowIPServicePrivate *priv;
  GMainLoop            *main_loop;
  guint                 signal_id;
...
GError *
flow_ip_service_get_last_error (FlowIPService *ip_service)
{
  FlowIPServicePrivate *priv;
  GError               *error = NULL;
