...
{                                                                                 \
  self_type *self = (self_type *) object;                                         \
                                                                                  \
  if (sizeof (self_type##Private) > 0)                                            \
    self->priv = g_type_instance_get_private ((GTypeInstance *) object, self_prefix##_get_type ()); \
                                                                                  \
...
/* Usage: FLOW_GOBJECT_MAKE_IMPL_NO_PRIVATE (flow_gadget, FlowGadget, G_TYPE_OBJECT, G_TYPE_FLAG_ABSTRACT) */

#define FLOW_GOBJECT_MAKE_IMPL_NO_PRIVATE(self_prefix, self_type, parent_type, reg_flags) \
static GObjectClass *self_prefix##_parent_class;                                  \
                                                                                  \
/* User funcs; user will have to define these */                                  \
static void self_prefix##_type_init (GType type);                                 \
